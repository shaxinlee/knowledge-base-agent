...
### 1.5.1 已批准范围变更：文档结构化摘要

自 2026-06-21 起，v0.1 增加文档结构化摘要能力，作为上述“实体抽取”排除项的
受限例外。该能力仅用于 Chunk 知识抽取、文档摘要展示和后续数据准备：

* 对每个 active Chunk 调用配置的 OpenAI-compatible LLM/vLLM，保存结构化
  `semantic_role`、`short_summary`、`topics`、`keywords`、`entities`、
  `assertions`、`importance` 和 `quality_flags`。
* 同一文档的 Chunk 使用有界并发处理，默认单文档 8 路、全局 16 路。
* 全部成功的 `short_summary` 必须按 `chunk_index` 原始顺序归并为文档摘要。
* 超过模型上下文预算时使用连续批次的分层归并，不允许截断文档尾部。
* 摘要使用独立状态和重试机制，不阻断文档 embedding、索引或 `indexed` 状态。
* 允许部分 Chunk 失败，并基于其余成功结果生成 `partially_completed` 摘要。
* 本例外不实现关系抽取、GraphRAG 召回、社区发现或图谱可视化，也不将摘要自动
  写入现有 Qdrant、BM25 或 Chat 上下文。

### 1.5.2 已批准范围变更：摘要关系图与知识库社区摘要

自 2026-06-21 起，v0.1 在文档结构化摘要之上增加受限的摘要关系图能力：

* 使用现有 Embedding 服务对完成的文档摘要生成文档级向量。
* 使用余弦相似度计算文件之间的摘要关联，每个文档仅保留超过配置阈值的 Top-K
  关系，默认阈值为 0.45、最多 6 条。
* 关系可以连接同一知识库或不同知识库下的文件，并明确标记跨知识库边。
* 用户和管理员均可通过知识地图页面只读探索文档节点、关系边和社区摘要；管理员
  可触发关系刷新或强制重算向量。
* 每个 active 知识库维护一份社区摘要，生成上下文仅来自该知识库当前文件的文档
  摘要；文档摘要集合变化时通过指纹自动更新。
* 当用户询问“这个知识库讲什么”“知识库整体概览”等总结性问题时，Chat 的整体
  概览上下文必须包含知识库创建时间、当前社区摘要，以及当前知识库下每个未删除
  文件的文档摘要或摘要状态；该路径不依赖 Chunk Top-K 召回。
* 图谱状态、文档摘要向量、关系边和社区摘要独立持久化，失败不得影响文件索引、
  检索或 Chat 主链路。
* 本能力不修改现有 RAG 召回，不执行实体关系抽取、子图召回或社区级问答路由。
