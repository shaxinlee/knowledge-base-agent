...
范围变更说明：自 2026-06-21 起，实体抽取的排除项增加“文档结构化摘要”受限
例外。测试范围包含逐 Chunk JSON schema 校验、原文 evidence 校验、有界并发、
断点恢复、分层摘要、部分完成状态、管理端摘要 API 和文件管理页摘要抽屉；关系
抽取、GraphRAG 召回和图谱可视化仍在范围外。

知识库整体概览问答还必须验证：类似“这个知识库讲什么的”能够命中整体概览路由，
回答包含知识库创建时间、社区摘要和当前知识库下每个文件的文档摘要；摘要尚未
生成、部分完成或失败时应返回稳定状态说明，且不得因 Top-K 检索遗漏文档。
- 文件版本管理。
- 文档级权限。
...
| ID | 优先级 | 场景 | 预期 |
| --- | --- | --- | --- |
| TDD-SUM-001 | P0 | 单文档 Chunk 并发 | 默认最多 8 个 Chunk 请求同时在途，且实际并发大于 1 |
| TDD-SUM-002 | P0 | 全局并发 | 多文档合计不得超过 16 个在途模型请求，重试同样受限 |
| TDD-SUM-003 | P0 | JSON 与证据校验 | 非法枚举、错误 chunk_id、越界 importance、非连续 evidence 被拒绝 |
| TDD-SUM-004 | P0 | 顺序归并 | 模型响应乱序时最终输入仍按 chunk_index 排列 |
| TDD-SUM-005 | P1 | 超长文档 | 使用连续批次分层归并，所有 Chunk 均被覆盖 |
| TDD-SUM-006 | P0 | 部分失败 | 成功结果生成摘要，状态为 partially_completed，文件仍可 indexed |
| TDD-SUM-007 | P0 | 恢复与幂等 | 重启后跳过已完成 Chunk，过期 parse job 不能覆盖当前摘要 |
| TDD-SUM-008 | P1 | 管理端展示 | 文件管理页可查看进度、摘要、失败原因并执行重试或强制重建 |

摘要关系图与社区摘要专项验收门槛：

| ID | 优先级 | 场景 | 预期 |
| --- | --- | --- | --- |
| TDD-GRAPH-001 | P0 | 文档摘要向量 | 仅对当前 active 文件的 completed/partially_completed 摘要生成向量并缓存 |
| TDD-GRAPH-002 | P0 | 相似度关系 | 使用余弦相似度，按阈值和每文档 Top-K 生成无重复无向边 |
| TDD-GRAPH-003 | P0 | 跨知识库关系 | 不同 active 知识库文档可建立关系并标记 cross_knowledge_base=true |
| TDD-GRAPH-004 | P1 | 自动更新 | 文档摘要内容、文件可见性或知识库状态变化后，全局指纹变化并触发重建 |
| TDD-GRAPH-005 | P0 | 社区摘要 | 每个 active 知识库根据当前文档摘要集合生成并按指纹更新社区摘要 |
| TDD-GRAPH-006 | P0 | 权限 | Admin/User 均可读取知识地图，只有 Admin 可触发刷新和强制重算 |
| TDD-GRAPH-007 | P1 | 页面探索 | 支持知识库筛选、跨库边开关、相似度阈值、节点选择、相关文档和社区摘要 |
| TDD-GRAPH-008 | P0 | 失败隔离 | 图谱或社区摘要失败不得改变文件 indexed 状态或影响现有 RAG |
