...
def is_openai_compatible_url(base_url: str) -> bool:
    """Detect if the base_url is OpenAI/vLLM compatible format."""
    normalized = base_url.lower().rstrip("/")
    return normalized.endswith("/v1") or normalized.endswith("/v1/embeddings")
...
class QwenMultimodalEmbeddingProvider(BaseEmbeddingProvider):
    def __init__(
        ...
        self._openai_mode = is_openai_compatible_url(self.base_url)
    ...
    def embed_batch(self, requests: list[EmbeddingRequest]) -> list[EmbeddingResult]:
        if not requests:
            return []
        # OpenAI/vLLM mode doesn't require API key
        if not self._openai_mode and not self.api_key:
            raise ApiError(
        ...
        try:
            with httpx.Client(timeout=self.timeout_seconds, transport=self.transport) as client:
                if self._openai_mode:
                    url, payload = self._build_openai_request(requests)
                else:
                    url = build_qwen_embedding_url(self.base_url)
    ...
    def _build_openai_request(
        ...
        """Build OpenAI/vLLM compatible request for text embeddings."""
        ...
        # vLLM expects simple strings or list of strings, not object arrays
        texts = [req.content for req in requests if req.input_type == "text"]
        
        # If single text, use string format; otherwise use array format
        if len(texts) == 1:
            payload = {
                "model": self.model_name,
                "input": texts[0],
