...
def delete_file(
    ...
    *,
    ...
    vector_index_client.deactivate_points(
        point_ids=[str(chunk_id) for chunk_id in active_chunk_ids]
    )
    bm25_index_client.deactivate_chunks(chunk_ids=[str(chunk_id) for chunk_id in active_chunk_ids])
    ...
    create_audit_log(
        ...
        details={
            "before": before,
            "inactive_chunk_count": len(active_chunk_ids),
            "qdrant_points_deactivated": len(active_chunk_ids),
            "qdrant_collection": vector_index_client.collection_name,
            "bm25_documents_deactivated": len(active_chunk_ids),
