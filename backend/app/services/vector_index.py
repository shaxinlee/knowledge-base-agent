...
class QdrantVectorIndexClient:
    ...
    def deactivate_points(self, *, point_ids: list[str]) -> None:
        ...
        # Check if collection exists; if not, nothing to deactivate
        collection_url = f"{self.base_url}/collections/{self.collection_name}"
        try:
            check = httpx.get(collection_url, timeout=self.timeout_seconds)
    ...
    def delete_points(self, *, point_ids: list[str]) -> None:
        ...
        # Check if collection exists; if not, nothing to delete
        collection_url = f"{self.base_url}/collections/{self.collection_name}"
        try:
            check = httpx.get(collection_url, timeout=self.timeout_seconds)
            if check.status_code == 404:
        ...
        try:
            ...
            raise ApiError(
                code="UPSTREAM_SERVICE_ERROR",
                message="Qdrant point deletion failed.",
                status_code=502,
                details={
                    "service": "qdrant",
                    "collection": self.collection_name,
