...
class KnowledgeGraphWorker:
    ...
    async def reload_llm_client(self) -> None:
        """Close the existing LLM client and create a new one with current settings."""
        ...
        if not base_url or not model:
            logger.warning(
                "Knowledge graph worker LLM client could not be reloaded because LLM base URL "
        ...
        logger.info(
            "Knowledge graph worker LLM client reloaded to model '%s' at %s.",
            model,
            base_url,
        )
